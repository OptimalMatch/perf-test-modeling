...
class MarketDataClientTest {
    ...
    void shouldCacheMarketData() {
        ...
        assertEquals(1.0, meterRegistry.counter("moo.marketdata.cache.miss").count());
        assertEquals(0.0, meterRegistry.counter("moo.marketdata.cache.hit").count());

        // Second call: cache hit
        client.getMarketData("MSFT");
        assertEquals(1.0, meterRegistry.counter("moo.marketdata.cache.miss").count());
        assertEquals(1.0, meterRegistry.counter("moo.marketdata.cache.hit").count());

        // Third call: still cache hit
        client.getMarketData("MSFT");
        assertEquals(1.0, meterRegistry.counter("moo.marketdata.cache.miss").count());
        assertEquals(2.0, meterRegistry.counter("moo.marketdata.cache.hit").count());
    }
