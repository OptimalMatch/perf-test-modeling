...
public class MarketCrashLoadTest {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("═══════════════════════════════════════════════════════");
        System.out.println("  MOO SOR Market Crash Load Test (2M webhooks)");
        System.out.println("═══════════════════════════════════════════════════════");
        ...
        for (String url : BASE_URLS) {
            ...
            double[] mongoLookup = getTimerStats(healthClient, url, "moo.mongo.lookup.time");
            totalMongoLookupTime += mongoLookup[0]; totalMongoLookupCount += (long) mongoLookup[1];

            double[] mongoUpdate = getTimerStats(healthClient, url, "moo.mongo.update.time");
        ...
        // Write to file
        try (var writer = new java.io.FileWriter("/tmp/moo-market-crash-report.txt")) {
            writer.write(report);
        }
        System.out.println("Report saved to /tmp/moo-market-crash-report.txt");
        System.exit(0);
    }
