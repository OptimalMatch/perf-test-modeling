...
public class LoadTestRunner {
    ...
    private final List<String> targetUrls; // base URLs of WID SOR instances
    ...
    public void waitForProcessingToComplete(String baseUrl, long expectedTotal, Duration timeout) {
        ...
        while (Instant.now().isBefore(deadline)) {
            try {
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create(baseUrl + "/actuator/metrics/wid.alert.processed"))
                        .GET()
    ...
    private long getTotalHandled(String baseUrl) {
        try {
            ...
            String[] metrics = {
                    "wid.alert.processed",
                    "wid.alert.skipped.inactive",
                    "wid.alert.skipped.throttled",
                    "wid.alert.skipped.not_found",
                    "wid.alert.failed"
            };
    ...
    public void collectMetricsIntoReport(PerformanceReportGenerator report, String baseUrl) {
        report.setAlertsProcessed(getMetricCount(baseUrl, "wid.alert.processed"));
        report.setAlertsSkippedInactive(getMetricCount(baseUrl, "wid.alert.skipped.inactive"));
        report.setAlertsSkippedThrottled(getMetricCount(baseUrl, "wid.alert.skipped.throttled"));
        report.setAlertsSkippedNotFound(getMetricCount(baseUrl, "wid.alert.skipped.not_found"));
        report.setAlertsFailed(getMetricCount(baseUrl, "wid.alert.failed"));

        report.setAvgMongoLookupMs(getMetricMeanMs(baseUrl, "wid.mongo.lookup.time"));
        report.setAvgMongoUpdateMs(getMetricMeanMs(baseUrl, "wid.mongo.update.time"));
        long mongoLookups = getMetricCount(baseUrl, "wid.mongo.lookup.time");
        long mongoUpdates = getMetricCount(baseUrl, "wid.mongo.update.time");
        report.setTotalMongoOps(mongoLookups + mongoUpdates);

        long cacheHits = getMetricCount(baseUrl, "wid.marketdata.cache.hit");
        long cacheMisses = getMetricCount(baseUrl, "wid.marketdata.cache.miss");
        double hitRatio = (cacheHits + cacheMisses) > 0 ? (cacheHits * 100.0 / (cacheHits + cacheMisses)) : 0;
        report.setCacheHitRatio(hitRatio);
        report.setAvgFetchTimeMissMs(getMetricMeanMs(baseUrl, "wid.marketdata.fetch.time"));

        report.setKafkaMessagesPublished(getMetricCount(baseUrl, "wid.alert.processed"));
        report.setAvgKafkaPublishMs(getMetricMeanMs(baseUrl, "wid.kafka.publish.time"));

        report.setAvgOrchestrationTimeMs(getMetricMeanMs(baseUrl, "wid.orchestration.time"));
        report.setCallerRunsCount((long) getGaugeValue(baseUrl, "wid.threadpool.caller.runs.count"));
